	...
	with (FBL) {
		...
		Firebug.firefinderModel = extend(Firebug.Module, {
			...
			addStyleSheet : function (doc) {
				...
				if (!styleSheet) {
					styleSheet = createStyleSheet(doc, "chrome://automan/skin/firebug.css");
		   	...
		   	run : function (context, element) {
				var panel = context.getPanel(panelName),
					...
					parse = function () {
						...
						if (matchingElements.length > 0) {
							for (var j=0, jl=matchingElements.length, elm, nodeNameValue, nodeNameCode, k, attr; j<jl; j++) {
								...
								resultItem += "<div class='firefinder-friendly-fire-this'>发送给AutoMan</div>";
					...
					if (!cssApplied) {
						...
						css.type = "text/css";
						css.rel = "stylesheet";
						css.href = "chrome://automan/content/browser.css";
